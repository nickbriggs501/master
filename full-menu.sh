...
do
    ...
    do
      ...
      then
        ...
        then
          ...
          sudo airodump-ng wlan0mon
          echo " "
          echo "press y to repair wifi or n to exit"
          echo " "
          read -n 1 -p "repair: " user_r
          if [ $user_r == y ]
          then
            sudo airmon-ng stop wlan0mon
            sleep 2
      ...
      then
        ...
        sleep 1
        echo $(tput setaf 2)" "
        echo "$(tput setaf 1)warning $(tput setaf 2)this script assumes rockyou.txt is located in the"
        echo "home directory, if not then use the custom wordlist option"
        echo " "
        echo "Select an option to continue"
        echo " "
        echo $(tput setaf 1)"1)$(tput setaf 2) enter known target ip address"
        echo $(tput setaf 1)"2)$(tput setaf 2) scan for connected devices"
        echo " "
        read -n 1 -p "option: $(tput setaf 1)" user_intent
        echo " "
        if [ $user_intent == 1 ]
        then
          sleep 1
          echo " "
          echo $(tput setaf 2)"enter the $(tput setaf 1)target's$(tput setaf 2) ip address below"
          echo " "
          read -p "target ip: $(tput setaf 1)" user_target
          echo " "
          echo " "
        ...
        then
          ...
          echo $(tput setaf 4)"======================================================="
          echo " $(tput setaf 2)"
          echo "please locate your $(tput setaf 1)inet ip$(tput setaf 2) address from above"
          echo " "
          echo "$(tput setaf 1)replace$(tput setaf 2) the $(tput setaf 1)last set$(tput setaf 2) of numbers with $(tput setaf 1)00"
          echo "$(tput setaf 2)and then enter it below in this format."
          echo " "
  ...
  then
    ...
    echo "x) exit"
    echo " "
    read -n 1 -p "connection: " user_c
    if [ $user_c == 1 ]
    then
      clear
      echo " "
      echo "known devices examples:"
      echo " "
      echo "pi@192,168.43.254  - Pi-tail"
      echo "pi@192.168.43.243  - Raspbian"
      echo "root@172.24.0.1    - P4wnP1"
      echo "pi@172.24.0.1      - P4wnP1"
      echo " "
      read -p "user name: " user_n
      read -p "ip address: " user_add
      ssh $user_n@$user_add
    fi
    if [ $user_c == 2 ]
    then
      ...
      read -p "user name: " user_n
      read -p "ip address: " user_add
      read -p "file/s: " user_f
      read -p "destination: " user_d
      scp $user_f $user_n@$user_add:$user_d
    fi
    if [ $user_c == 3 ]
    then
      clear
      echo " "
      read -p "target ip: " user_ftp
      ftp $user_ftp
    fi
    if [ $user_c == 4 ]
    then
  ...
  then
    ...
    echo "1) enable ftp"
    echo "2) disable ftp"
    echo "3) enable ssh server"
    echo "4) disable ssh server"
    echo "5) enable full ssh access"
    echo "6) enable vnc server (desktop)"
    echo "7) disable vnc server (desktop)"
    echo " "
    read -n 1 -p "enable: " user_en
    if [ $user_en == 1 ]
    then
      sv-enable ftpd
      sv up ftpd
  ...
  then
    ...
    echo "5) notes"
    echo " "
    read -n 1 -p "destination: " user_dest
    if [ $user_dest == 1 ]
    then
  ...
  then
    clear
    echo "enter the number in the following format"
    echo "1-603-xxx-xxxx"
    echo " "
    read -p "number: " user_number
    cd $HOME/PhoneInfoga
  ...
  then
    echo "1) cupp"
    echo "2) lazybee"
    echo "3) view finished pwd lst"
    read -n 1 -p "opt: " user_pwdlst
    if [ $user_pwdlst == 1 ]
    then
      cd $HOME/cupp
      python cupp.py -i
  ...
  then
    ...
    then
      ...
      echo " "
      read -n 1 -p "continue?: " user_ready
      if [ $user_ready == y ]
      then
        clear
        echo " "
    ...
    then
      ...
      echo " "
      read -n 1 -p "proceed? y/n " user_pro
      if [ $user_pro == y ]
      then
